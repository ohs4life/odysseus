        ...
        if _last_user_text.strip():
            ...
            if _kb_outcome == "error":
                ...
                _kb_text = (
                    ...
                    "science, current events, geography, sports, technology, "
                    "documentation walkthroughs, etc.) that is clearly NOT about "
                    "OHS, you MUST immediately call the `web_search` or `web_fetch` "
                    "tool. Do NOT emit placeholder text like \"Reference context "
                    "received.\" Do NOT ask the user for permission — just call "
                    "the tool. Cite the source URL(s) you used in your reply, and "
                    "the answer must be grounded in the web result.\n"
                    ...
                    "validated answer for general topics. If your web tools aren't "
                    "available this turn, say so explicitly so the user knows to try "
                    "again in a session that has web tools enabled.\n"
                    "- When the user has just told you to \"web search\" or "
                    "\"look it up\", treat that as an explicit command: do the "
                    "search immediately, do not refuse, do not ask permission."
                )
